...
if [ $(ulimit -n) -lt ${MAX_OPEN_FILES} ]; then
    echo -e "The current open files limit is set to less than ${MAX_OPEN_FILES} \nAttempting to increase limit..."
    if ulimit -n ${MAX_OPEN_FILES}; then
        echo "Limit increased to ${MAX_OPEN_FILES} open files"
    else
        echo "Failed to increase open file limit; you may experience problems under heavy load"
    fi
fi
